...
const show = (options) => (view, ctx = {}) => {
    let opts = (0, record_1.merge)(defaultOpts, options);
    let env = nunjucks.configure(opts.path, opts);
    // XXX: We leave it to the user to specify ensure filters are proper
    // functions.
    if ((0, record_1.isRecord)(opts.filters))
        (0, record_1.forEach)(opts.filters, (f, k) => env.addFilter(k, f));
    return (0, future_1.fromCallback)(cb => env.render(view, ctx, (e, content) => {
        if (e)
            return cb(e);
